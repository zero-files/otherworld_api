...
export default class Route {
    ...
    constructor(method:"get"|"post", path:string, levelAuth?:"user"|"readwrite"|"read", controller?:(req:Request, res:Response)=>void){
        this.method = method
        let defPath:string = path.startsWith("/") ? path : `/${path}`
        this.path = defPath
        this.authenticated = levelAuth ? authentication(levelAuth) : function(req:Request, res:ExResponse, next:NextFunction) { return next() }
